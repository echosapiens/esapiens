...
interface ExportMethodsProps { sessionId: string; }
...
export function ExportMethods({ sessionId }: ExportMethodsProps) {
  ...
  const methodsText = generateMethodsText(activePipeline, runs ?? [], { includeTimestamps, includeDigests });
    ...
    <div className="card p-4">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Export Methods</h3>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleCopy} className="btn btn-ghost text-xs">
            {copied ? <><Check className="h-3.5 w-3.5" style={{ color: "var(--accent-green)" }} /> Copied!</> : <><Copy className="h-3.5 w-3.5" /> Copy</>}
          </button>
          <button onClick={handleDownload} className="btn btn-primary text-xs">
      ...
      <div className="mb-4 flex items-center gap-4">
        ...
        <label className="flex items-center gap-2 text-xs" style={{ color: "var(--text-secondary)" }}>
          <input type="checkbox" checked={includeDigests} onChange={(e) => setIncludeDigests(e.target.checked)}
            className="rounded" style={{ accentColor: "var(--accent-gold)" }} />
          Include container digests
        </label>
      </div>

      <div className="max-h-96 overflow-auto rounded-lg p-4" style={{ background: "var(--bg-surface)" }}>
        <pre className="whitespace-pre-wrap font-mono text-xs leading-relaxed" style={{ color: "var(--text-secondary)" }}>
...
function generateMethodsText(pipeline: PipelineRead | null, runs: RunRead[], options: { includeTimestamps: boolean; includeDigests: boolean }): string {
  if (!pipeline) return "";
  ...
  lines.push("=====================");
  lines.push("");
  lines.push(`Bioinformatics analyses were performed using the E.sapiens platform (v0.1.0). The pipeline "${pipeline.name}" was executed with the following containerized tools.`);
  lines.push("");
  ...
  if (Array.isArray((dag as Record<string, unknown>)?.steps)) {
    ...
    for (const [name, val] of Object.entries(dag)) {
      if (typeof val === "object" && val !== null) steps.push({ name, ...(val as Record<string, unknown>) });
    }
  }
  for (const step of steps) {
    ...
    if (containerImage) {
      ...
      if (options.includeDigests && containerImage.includes("@")) {
        ...
        if (digest) lines.push(`  Container image digest: ${digest}`);
      }
    }
    if (commandArgs.length > 0) lines.push(`  Command-line arguments: ${commandArgs.join(" ")}`);
    if (run && options.includeTimestamps) {
      if (run.started_at) lines.push(`  Execution started: ${formatTimestamp(run.started_at)}`);
      if (run.completed_at) lines.push(`  Execution completed: ${formatTimestamp(run.completed_at)}`);
  ...
  lines.push("### Summary");
  lines.push("");
  lines.push(`A total of ${totalRuns} bioinformatics step${totalRuns !== 1 ? "s were" : " was"} executed: ${completedRuns} completed successfully${failedRuns > 0 ? `, ${failedRuns} failed` : ""}.`);
  lines.push("");
  lines.push("All analyses were performed using containerized tools with pinned SHA256 digests to ensure reproducibility. Container images were sourced from the Biocontainers project (https://biocontainers.pro/).");
  return lines.join("\n");
}