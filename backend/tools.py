...
def _safe_import(name: str, *args, **kwargs):
    ...
    if root not in _ALLOWED_IMPORTS:
        raise ImportError(
            f"Sandbox restriction: '{root}' is not allowed. Allowed: {sorted(_ALLOWED_IMPORTS)}"
        )
...
@timed
def download_pdb(
    pdb_id: str, format: str = "pdb", representation: str = "cartoon"
) -> ToolResult:
    ...
    """
    WORKSPACE = Path(
        os.environ.get("ESAPIENS_DATA_DIR", os.path.expanduser("~/esapiens-data"))
    )
    ...
    try:
        ...
        if resp.status_code == 404:
            return ToolResult.err(
                "download_pdb", f"PDB ID '{pdb_id}' not found at RCSB."
            )
        ...
        return ToolResult.err(
            "download_pdb", f"Timeout fetching PDB '{pdb_id}' from RCSB."
        )
    except httpx.HTTPStatusError as e:
        return ToolResult.err(
            "download_pdb", f"RCSB returned {e.response.status_code} for '{pdb_id}'."
        )
...
def parse_structure(file_path: str) -> ToolResult:
    ...
    try:
        ...
        if not lines:
            return ToolResult.err(
                "parse_structure", f"No ATOM/HETATM records in {file_path}"
            )
        residues = set()
        atoms = 0
        for l in lines:
...
def search_literature(
    query: str, source: str = "all", max_results: int = 10
) -> ToolResult:
            ...
            if src == "web":
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        return ToolResult.err(
                            "search_literature",
                            "BRAVE_SEARCH_API_KEY is invalid or expired.",
                        )
                    else:
                        results.append(
                            {
                                "source": "web",
                                "error": f"Brave API returned {resp.status_code}",
                            }
                        )
                except Exception as e:
...
def _search_literature_fallback(
    query: str, source: str, max_results: int
) -> ToolResult:
...
@timed
def download_tcga_survival(cohort: str, survival_type: str = "overall") -> ToolResult:
    ...
    try:
        ...
        return ToolResult.ok(
            "download_tcga_survival",
            data={
                "cohort": cohort,
                "num_patients": len(hits),
                "survival_type": survival_type,
            },
        )
    except httpx.TimeoutException:
        return ToolResult.err(
            "download_tcga_survival", f"GDC API timeout for cohort '{cohort}'."
        )
    except Exception as e:
...
@timed
def run_bio_pipeline(
    command: str, name: str, estimated_duration: str = "unknown"
) -> ToolResult:
...
def get_job_status(job_id: str) -> ToolResult:
    ...
    workspace = Path(
        os.environ.get("ESAPIENS_DATA_DIR", "~/esapiens-data")
    ).expanduser()
    job_path = workspace / "background_jobs" / f"{job_id}.json"
    if not job_path.exists():
        return ToolResult.err(
            "get_job_status",
            f"Job '{job_id}' not found. No record in storage or at '{job_path}'.",
        )
    try:
...
def run_python_plot(code: str, title: str = "", pretty: bool = True) -> ToolResult:
    ...
    try:
        ...
        exec(
            compile(
                code.strip("`").replace("python", "", 1), "<run_python_plot>", "exec"
            ),
            {
        ...
        if not fig.axes:
            return ToolResult.err(
                "run_python_plot", "No axes generated. Check your plotting code."
            )
        buf = io.BytesIO()
...
def plotly_plot(code: str, title: str = "Plot") -> ToolResult:
    ...
    try:
        ...
        if not fig:
            return ToolResult.err(
                "plotly_plot", "No 'fig' object found. Return fig from your code."
            )
...
def execute_python(code: str, description: str = "") -> ToolResult:
    ...
    _BIO_PATTERNS = (
        ...
        r"\bfrom\s+scanpy\b",
        r"\bimport\s+biopython\b",
        r"\bfrom\s+Bio\b",
        r"\bimport\s+Bio\b",
        r"\bimport\s+pybedtools\b",
        r"\bimport\s+pysam\b",
        r"\bfrom\s+pysam\b",
        r"\bimport\s+deeptools\b",
        r"\bfrom\s+deeptools\b",
    ...
    for pattern in _BIO_SEMANTIC_PATTERNS:
        if _re.search(pattern, code_lower):
            return ToolResult.err(
                "execute_python",
                f"Bioinformatics computation detected ('{pattern}' pattern). "
    ...
    for pattern in _TCGA_BIO_URLS:
        if _re.search(pattern, code, _re.IGNORECASE):
            return ToolResult.err(
                "execute_python",
                "TCGA/GEO genomic data portal access detected. "
    ...
    ns = {
        ...
        "pd": __import__("pandas", fromlist=[""]),
        "WORKSPACE": WORKSPACE,
    }
    try:
        exec(compile(code.strip("`").replace("python", "", 1), "<exec>", "exec"), ns)
        return ToolResult.ok(
            "execute_python", data={"description": description or "code executed"}
        )
    except Exception as e:
...
def run_modal_job(job_type: str, params: dict, background: bool = False) -> ToolResult:
    ...
    if not MODAL_AVAILABLE:
        return ToolResult.err(
            "run_modal_job", "Modal SDK not installed or not importable."
        )
    if job_type not in BIO_TASKS:
        return ToolResult.err(
            "run_modal_job",
            f"Unknown job_type '{job_type}'. Available: {list(BIO_TASKS.keys())}",
        )
    workspace = Path(
        os.environ.get("ESAPIENS_DATA_DIR", "~/esapiens-data")
    ).expanduser()
...
def find_biocontainer(query: str, exact_match: bool = False) -> ToolResult:
    ...
    try:
        ...
        if resp.status_code not in (200, 404):
            return ToolResult.err(
                "find_biocontainer", f"Quay.io API returned {resp.status_code}"
            )
        ...
        for repo in repos:
            ...
            try:
                ...
                if tags_resp.status_code == 200:
                    tags = [
                        t.get("name") for t in tags_resp.json().get("tags", []) or []
                    ]
            except Exception:
                tags = []
...
def run_custom_script(
    ...
    if not MODAL_AVAILABLE:
        return ToolResult.err(
            "run_custom_script", "Modal SDK not installed on this server."
        )
...
from dynamic_tools import (
    create_tool as _create_tool_impl,
    load_dynamic_tools as _load_dynamic_tools,
)  # noqa: E402
...
def execute_tool(name: str, args: dict) -> str:
    ...
    if name not in TOOL_IMPLS:
        err = ToolResult.err(
            "execute_tool",
            f"Unknown tool: '{name}'. Available: {list(TOOL_IMPLS.keys())}",
        )
    ...
    try:
        ...
        return json.dumps(
            {"tool": name, "status": "success", "data": result}, default=str
        )
    except Exception as e:
...
try:
    ...
    if loaded:
        print(
            f"[tools] Initialized {len(loaded)} persistent dynamic tools: {[t['name'] for t in loaded]}"
        )
except Exception as e:
    print(f"[tools] DYNAMIC TOOL LOADER FAILED on startup: {e}")
    print("[tools] Agent will continue.")
